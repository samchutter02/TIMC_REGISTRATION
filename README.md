...
### System Overview
This is a web-based registration system for the TIMC event. Users (directors, individuals, or admins) fill out a form to register groups or individuals for workshops, performances, and related activities. Key features:
...
- **Data Storage**: MySQL database stores directors, performers, groups, and songs.
- **Emails**: Automated confirmations sent to users, directors, and admins.
...
- **Email Setup**: SMTP server (code assumes localhost port 25, no auth; update for production).
...
1. **Clone/Extract Code**:
   - Visit my repo (ask for access it is private). Clone it.

2. **Install Dependencies**:
   - Run `composer install` to get Stripe and PHPMailer.

3. **Configure Environment (.env File)**:
   - Create `.env` in the root (no spaces/quotes in values):
     ```
     DB_HOST=localhost
     DB_NAME=timc_reg
     DB_USER=root
     ...
     DB_CHARSET=utf8mb4
     ```
   - This is loaded in `process.php` and `charge.php` for DB connections.

4. **Database Setup**:
   - Create database and tables (inferred from code; run this SQL):
     ```sql
     ...
     ```
   - Indexes: Add UNIQUE on `groups.group_name`

5. **Stripe Keys**:
   - In `charge.php`: Replace `sk_test_h**********************` with your secret key.
   - In `invoice.php`: Replace `pk_test_5**********************` with your publishable key.
...
8. **Testing**:
   ...
   - Test flows: Group/Individual, Mariachi/Folklorico, CC/PO.
...
1. **Access Form**: User visits `index.php` (main form).
   - If `.reg-closed` exists, shows closure message.
2. **Fill Form**:
...
5. **Confirmation**:
   - Emails sent to director, user (if user isn't the directr), and admin.
...
**Purpose**: The entry point and core UI. Renders a multi-section (non-paginatedD) form for data entry. Uses JS for dynamic behavior (e.g., adding performer rows, toggling fields).
...
- **Submission**: Posts to `process.php`.
...
**Purpose**: Handles form submission from `index.php`. Validates, calculates, and either inserts (for PO) or prepares for payment (CC).
...
**Purpose**: Contains functions for sending confirmation and resume emails (resume not used in provided code).
...
- **sendResumeLink**: Similar, for resuming (not implemented in other files).
...
- **Testing**: Use Stripe test cards (e.g., 4242424242424242).
